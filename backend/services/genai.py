...
class YoutubeProcessor:
    ...
    def retrieve_youtube_documents(self, video_url: str, verbose = False):
        ...
        if verbose:
            #if print is used instead of logger, the results do not actually print in production
                #therefore, must use logger.info
    ...
    def find_key_concepts(self, documents: list, sample_size: int = 0, verbose = False):
        ...
        if sample_size > len(documents):
         ...
         # Optimization sample size given no input
        if sample_size == 0:
        ...
        for group in tqdm(groups):
            ...
            prompt = PromptTemplate(
                ...
                Return valid json and respond in the following format as a string separating each concept with a comma:
        ...
        for concept in batch_concepts:
            ...
            #Try to upload the concept as is to output a Python Dict
            try:
                concept_dict = json.loads(concept)
                
                #Check that Python Dict is not empty and then append.
                if concept_dict is not None and concept_dict != "[]":
                    processed_concepts.append(concept_dict)

            #For json strings that are not coverted to a Python Dict
            except json.JSONDecodeError as e:

                #Check to see concept is not empty and modify by replacing backticks, 'json', and backslash are removed and no leading/trailing spaces
                if concept is not None and concept != "[]":
                    pattern = r'\{"[^"]+":\s*"[^"]+"\}(?:,\s*\{"[^"]+":\s*"[^"]+"\})*'
                    mod_match = re.findall(pattern, concept)
                    mod_concept = json.dumps(mod_match)

                    #Check to see if the modified concept is not empty.
                    if mod_concept is not None and mod_concept != "[]":
                        concept_dict = json.loads(mod_concept)
